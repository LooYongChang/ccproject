...
$adm_name =  $_SESSION['v_name'];
$adm_id = $_SESSION['v_id'];
...
if (isset($_POST['smtbtn'])) {
    ...
    if (!empty($getcode)) {
        $result = mysqli_query($connect, "SELECT * FROM admin WHERE admin_id = '$adm_id'");
        $row = mysqli_fetch_assoc($result);

        if ($row['admin_vericode'] == $getcode) {
?> <script>
                alert("Please Reset your password!");
            </script> <?php
                        header("refresh:0.2, url=aresetpassword.php");
                    } else {
                        ?>
            <script>
